...
def test_read_csv_no_header(tmp_path: Path):
    p = tmp_path / "data.csv"
    p.write_text("1,2\n3,4\n", encoding="utf-8")
    df = read(str(p), delimiter=",", header=["A", "B"])
    assert list(df.columns) == ["A", "B"]
    assert df.iloc[0].to_dict() == {"A": "1", "B": "2"}
...
def test_read_excel_with_header(tmp_path: Path):
    ...
    df = read(str(p), sheet=0, encoding="utf-8")
    assert df.loc[0, "A"] == "1"
    assert df.loc[1, "A"] == "3"
    assert df.loc[0, "B"] == "2"
    assert df.loc[1, "B"] == "4"
...
def test_read_excel_no_header(tmp_path: Path):
    p = tmp_path / "data.xlsx"
    pd.DataFrame([["1", "2"], ["3", "4"]]).to_excel(p, index=False, header=False)
    df = read(str(p), header=["A", "B"])
    assert df.loc[0, "A"] == "1"
    assert df.loc[1, "A"] == "3"
    assert df.loc[0, "B"] == "2"
    assert df.loc[1, "B"] == "4"
